...
public class PlayerCtrl : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // 점프 종료
        if (isJumping && !Input.GetButton("Jump") || Time.time - jumpStartTime >= maxJumpTime)  // 점프 버튼을 떼거나 시간이 지나면 점프 종료
        {
            isJumping = false;
    ...
    void UpdateColliderSize() // 콜라이더 업데이트 일반마리오 상태 또는 슈퍼 마리오 상태
    {
        if (sizeUp)
        {
            ...
            itemCol.offset = new Vector2(itemCol.offset.x, 0.24f);
        }
        else
        {
            // 원래 크기로 복구
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Enemy") && flowerState && !starState) // 플라워 상태일 때 적과 충돌하면 기본 마리오 상태로
        {
